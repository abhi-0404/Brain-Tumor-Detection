...
### Conclusion
The project explores three different models for brain tumor detection. Each model's performance is evaluated based on loss, accuracy, F1-score, and confusion matrix. The best-performing model can be selected for deployment through the CLI or the Flask app in `web_app/`.
...
- The notebook is the main entry point for training, evaluation, and comparison of the three models.
- The notebook expects `Dataset/Training` and `Dataset/Testing`.
- Save the best model artifact in `artifacts/best_model.keras` if you plan to reuse it in the CLI or web app.
- For repeatable runs, prefer `python -m brain_tumor_detection.train`.

## Recommended Run Order

1. Load imports and helper functions.
2. Build train and test dataframes from local folders.
3. Create train/validation/test generators.
4. Train CNN, then MLP, then VGG16.
5. Run evaluation and confusion matrix cells.
6. Save and reload the best model artifact.
